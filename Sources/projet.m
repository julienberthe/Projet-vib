...
disp('I	Construction du probleme mecanique EF');
	...
	donnee.npas  = 5000;	%nombre de pas de temps

	%caracteristique du probleme
	donnee.T= 1;	%temps d'etude
...
disp('III	Construction du chargement');
    ...
    disp('==============R�solution EF=============');
    disp('========================================');
    option.schema='euler AR';  %%champs possibles 'euler AR', 'euler AV' (non impl�ment�) ou 'newmark' (non impl�ment�)
    option.resolution='modale'; %%champs possibles 'modale' ou 'directe'
    %nombre de modes propres � pendre en compte
    option.Nb_ef=50;
    [ModePropre,U,Eps]=EF(chargement,matriceS,donneeS,option);
    
    
    %%%%%Affichages divers et vari�s
...
%    Affichage(ModePropre,donneeS,option);
    ...
    option.type='en fonction du temps';
	option.titre='d�placement extr�mit� de la poutre en fct du tps';
	Affichage(U.U(donnee.nelem+1,:),donneeS,option)
    option.titre='vitesse extr�mit� de la poutre en fct du tps';
    %Affichage(U.V(donnee.nelem+1,:),donnee,option)
    
    
    option.titre='acc�l�ration extr�mit� de la poutre en fct du tps';
    %Affichage(U.A(donnee.nelem+1,:),donnee,option)
    
    option.type='animation en fonction du temps';
	option.titre='D�placement des points en fct du temps';
    ...
    option.type='3D';
    option.titre='D�placement de la poutre en fonction du temps et de labscisse';
    ...
    option.titre='Déformation de la poutre en fonction du temps et de labscisse';
    Affichage(Eps,donnee,option)
